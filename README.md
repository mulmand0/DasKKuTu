...
- 폴더 `./Server/lib/sub`에 oAuth 설정 파일(`auth.inc.json`)과 전역 설정 파일(`global.inc.json`)을 **반드시** ('auth.json'), ('global.json)으로 변경해주세요. 본 레포지토리에는 각 파일의 양식이 들어가 있습니다.
...
- 랭킹 및 세션 기능 일부는 [Redis](https://redis.io/) 서버가 실행되어야만 작동합니다. 실행아지 않을 시, main.js쪽에 에러가 납니다.
...
- PR 적용 시 **무조건** 줄에 맞춰 일일히 수정해야합니다. 파일 자체를 다운로드 후 갈아엎는 것은 올바른 방법이 아닙니다.
- 닫힌 PR은 되도록이면 적용하지 마세요. 쪼리핑님 이외 관리자분들이 Apporoved를 안내리고, 리뷰에서 수정 요청 후 PR을 닫으면 오류가 있단 것 입니다. 적용할거면 소스 파일을 검토 후 적용하세요.
#### 폰트 변경 방법
